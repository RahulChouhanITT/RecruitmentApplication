...
const axiosBaseQuery = createAxiosBaseQuery<ChatApiResponse>("Chat request failed");
...
export const chatApi = createApi({
  ...
  endpoints: (builder) => ({
    getConversations: builder.query<ChatApiResponse<ChatConversation[]>, void>({
      queryFn: () => axiosBaseQuery<ChatConversation[]>({ url: "/api/chats/conversations", method: "GET" }),
      providesTags: (result) => [
    ...
    sendMessage: builder.mutation<ChatApiResponse<ChatMessage>, { conversationId: string; message: string }>({
      ...
      async onQueryStarted({ conversationId }, { dispatch, queryFulfilled }) {
        const patchConversations = dispatch(
          chatApi.util.updateQueryData("getConversations", undefined, (draft) => {
            ...
            const targetConversation = conversations.find((conversation) => conversation._id === conversationId);
    ...
    startCandidateHrConversation: builder.mutation<ChatApiResponse<ChatConversation>, void>({
      queryFn: () =>
        axiosBaseQuery<ChatConversation>({
