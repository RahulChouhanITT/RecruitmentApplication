...
const renderRequiredMark = () => <RequiredMark aria-hidden="true">*</RequiredMark>;
...
export const ProfileCompletionModal = ({
  ...
  const validate = (): boolean => {
    ...
    if (isCandidate) {
      ...
      if (!formValues.currentLocation?.trim()) {
        ...
        nextErrors.currentLocation = PROFILE_VALIDATION_MESSAGES.CURRENT_LOCATION_MAX_LENGTH(
  ...
  const onResumeFileChange = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    const hasAllowedExtension = PROFILE_RESUME_ACCEPTED_EXTENSIONS.some((extension) =>
    ...
    try {
      await uploadResume({ file: selectedFile }).unwrap();
      const refreshedProfileResponse = await refetchProfile();
      const resumeUrl = refreshedProfileResponse.data?.data?.resumeUrl?.trim() ?? '';
        ...
        <Card>
          ...
          {isCandidate ? (
            <Row>
              <Field>
                ...
                />
                <FieldError>
                  {fieldErrors.phone || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <LabelRow>
                  ...
                  <ResumeHeaderRow>
                    {hasResume ? (
                      <ResumeUploadedLink
                        type="button"
                        onClick={() =>
                          window.open(effectiveResumeUrl, '_blank', 'noopener,noreferrer')
                        }
                ...
                <FieldError>
                  {fieldErrors.resumeUrl || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <LabelRow>
                ...
                />
                <FieldError>
                  {fieldErrors.skills || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <LabelRow>
                ...
                />
                <FieldError>
                  {fieldErrors.experienceYears || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <LabelRow>
                ...
                />
                <FieldError>
                  {fieldErrors.currentLocation || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
            </Row>
          ) : null}
          ...
          {isHr ? (
            <Row>
              <Field>
                ...
                />
                <FieldError>
                  {fieldErrors.position || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <LabelRow>
                ...
                />
                <FieldError>
                  {fieldErrors.experienceLevel || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <LabelRow>
                ...
                />
                <FieldError>
                  {fieldErrors.department || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
            </Row>
          ) : null}
          ...
          {isInterviewer ? (
            <Row>
              ...
              <Field>
                ...
                />
                <FieldError>
                  {fieldErrors.techStack || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <LabelRow>
          ...
          <Actions>
            ...
            <PrimaryButton type="button" disabled={isSaveDisabled} onClick={onFormSubmit}>
              {isSubmitting
                ? PROFILE_UI_TEXT.SAVING
                : isUploadingResume
                  ? PROFILE_UI_TEXT.UPLOADING
                  : PROFILE_UI_TEXT.SAVE_PROFILE}
            </PrimaryButton>
          </Actions>
        </Card>
      </Overlay>
