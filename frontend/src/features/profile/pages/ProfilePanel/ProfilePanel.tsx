...
export const ProfilePanel = () => {
  ...
  const avatarText = useMemo(() => {
    const source =
      (isEditing ? formValues.name : profileData?.name) ||
      currentUser?.name ||
      PROFILE_INITIAL_VALUES.AVATAR_FALLBACK;
  ...
  if (!profileData) {
    return (
      <DashboardDescription>{PROFILE_DEFAULT_MESSAGES.PROFILE_LOAD_FAILED}</DashboardDescription>
    );
  }
  ...
  const effectiveResumeUrl = (formValues.resumeUrl ?? profileData.resumeUrl ?? '').trim();
  const hasResume = Boolean((profileData.resumeUrl ?? '').trim());
  const isResumeActionInProgress = isUploadingResume;

  const updateValue = <T extends keyof UpdateProfilePayload>(
    key: T,
    value: UpdateProfilePayload[T],
  ): void => {
  ...
  const onCandidatePhoneChange = (event: ChangeEvent<HTMLInputElement>): void => {
    const digitsOnlyValue = event.target.value
      .replace(/\D/g, '')
      .slice(0, PROFILE_FORM_LIMITS.phoneDigits);
  ...
  const onUploadResume = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    const hasAllowedExtension = PROFILE_RESUME_ACCEPTED_EXTENSIONS.some((ext) =>
      lowerCaseFileName.endsWith(ext),
    );
    const hasAllowedMimeType = PROFILE_RESUME_ACCEPTED_MIME_TYPES.includes(
      selectedFile.type as (typeof PROFILE_RESUME_ACCEPTED_MIME_TYPES)[number],
    );
  ...
  const validateProfileForm = (): boolean => {
    ...
    if (isCandidate) {
      ...
      if (!formValues.currentLocation?.trim()) {
        ...
        (formValues.currentLocation ?? '').trim().length > PROFILE_FORM_LIMITS.maxLocationLength
      ) {
        nextErrors.currentLocation = PROFILE_VALIDATION_MESSAGES.CURRENT_LOCATION_MAX_LENGTH_PANEL(
          PROFILE_FORM_LIMITS.maxLocationLength,
        );
      }
      if (!formValues.skills?.trim()) {
        ...
        nextErrors.skills = PROFILE_VALIDATION_MESSAGES.SKILLS_MAX_LENGTH(
          PROFILE_FORM_LIMITS.maxSkillsLength,
        );
      }
      if (
        ...
        nextErrors.experienceYears = PROFILE_VALIDATION_MESSAGES.EXPERIENCE_INVALID_PANEL(
          PROFILE_FORM_LIMITS.maxExperienceYears,
        );
      }
  ...
  const onSave = async (): Promise<void> => {
    ...
    try {
      const payloadToSave: UpdateProfilePayload =
        isCandidate && effectiveResumeUrl
          ...
          : formValues;
      const response = await updateProfile(payloadToSave).unwrap();
      dispatch(
        setCurrentUser(
          currentUser
            ? {
                ...currentUser,
                ...response.data,
                name: formValues.name?.trim() || response.data?.name || currentUser.name,
              }
            : (response.data ?? null),
        ),
      );
    ...
    <ProfileWrap>
      ...
      <HeaderCard>
        ...
        <HeaderInfo>
          <Name>
            {(isEditing ? formValues.name : profileData.name) ||
              PROFILE_INITIAL_VALUES.DISPLAY_FALLBACK}
          </Name>
          <Meta>
            {PROFILE_UI_TEXT.ROLE_PREFIX} {getRoleLabel(profileData.role)}
          </Meta>
          <Meta>
            {PROFILE_UI_TEXT.EMAIL_PREFIX} {profileData.email}
          </Meta>
        </HeaderInfo>
        <HeaderActions>
          {!isEditing ? (
        ...
        <Grid>
          ...
          <Field>
            ...
            <ErrorInput
              ...
              readOnly
              placeholder={PROFILE_UI_TEXT.EMAIL_PLACEHOLDER}
            />
          ...
          {isCandidate ? (
            <>
              <Field>
                ...
                />
                <FieldError>
                  {formErrors.phone || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <Label>
                ...
                />
                <FieldError>
                  {formErrors.currentLocation || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
            </>
          ) : null}
          ...
          {isCandidate ? (
            <>
              <Field>
                ...
                />
                <FieldError>
                  {formErrors.skills || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <Label>
                ...
                />
                <FieldError>
                  {formErrors.experienceYears || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              <Field>
                <Label>
                ...
                <ErrorResumeSection $hasError={Boolean(resumeActionError || formErrors.resumeUrl)}>
                  ...
                  {!hasResume ? (
                    <ResumeActions>
                      ...
                      >
                        {isUploadingResume
                          ? PROFILE_UI_TEXT.UPLOADING
                          : PROFILE_UI_TEXT.UPLOAD_RESUME}
                      </ResumeButton>
                    </ResumeActions>
                  ) : null}
                  <FieldError>
                    {resumeActionError ||
                      ...
                      PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                  </FieldError>
                </ErrorResumeSection>
              </Field>
            </>
            ...
            <>
              <Field>
                ...
                />
                <FieldError>
                  {formErrors.position || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                </FieldError>
              </Field>
              {isHr ? (
                <Field>
                  ...
                  <ErrorInput
                    ...
                    value={
                      isEditing ? (formValues.department ?? '') : (viewValues.department ?? '')
                    }
                    readOnly={!isEditing}
                  ...
                  />
                  <FieldError>
                    {formErrors.department || PROFILE_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
                  </FieldError>
                </Field>
              ) : null}
              <Field>
                ...
                <ErrorInput
                  ...
                  onChange={(event) =>
                    isHr
                      ...
                      : updateValue('techStack', event.target.value)
                  }
                  placeholder={
                    isHr
                      ? PROFILE_UI_TEXT.EXPERIENCE_LEVEL_PLACEHOLDER
                      : PROFILE_UI_TEXT.TECH_STACK_INPUT_PLACEHOLDER
                  }
                />
                <FieldError>
                  {isHr
