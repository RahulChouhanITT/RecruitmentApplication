...
import {
  ...
  Textarea,
  Title,
} from './InterviewerFeedbackModal.style';
...
export const InterviewFeedbackModal = ({
  ...
  const [comments, setComments] = useState<string>(
    INTERVIEWER_INITIAL_VALUES.FEEDBACK_FORM.comments,
  );
  ...
  const handleSubmit = async (): Promise<void> => {
    if (isLocalSubmitting || isSubmitting || !validate()) {
      return;
    }
      ...
      <Card>
        ...
        <Grid>
          <Label>
            ...
            >
              <option value={INTERVIEWER_INITIAL_VALUES.EMPTY_STRING}>
                {INTERVIEWER_UI_TEXT.RATING_PLACEHOLDER}
              </option>
              {[1, 2, 3, 4, 5].map((value) => (
          ...
          <Label>
            ...
            <Select
              ...
              onChange={(event) => {
                ...
                setErrors((prev) => ({
                  ...prev,
                  recommendation: INTERVIEWER_INITIAL_VALUES.EMPTY_STRING,
                }));
              }}
            >
              <option value={INTERVIEWER_INITIAL_VALUES.EMPTY_STRING}>
                {INTERVIEWER_UI_TEXT.RECOMMENDATION_PLACEHOLDER}
              </option>
              <option value={INTERVIEWER_STATUS_VALUES.HIRED}>
                {INTERVIEWER_UI_TEXT.RECOMMENDATION_HIRED}
              </option>
              <option value={INTERVIEWER_STATUS_VALUES.REJECTED}>
                {INTERVIEWER_UI_TEXT.RECOMMENDATION_REJECTED}
              </option>
            </Select>
            <ErrorText>
              {errors.recommendation || INTERVIEWER_UI_TEXT.FIELD_ERROR_PLACEHOLDER}
            </ErrorText>
          </Label>
        ...
        <Actions>
          ...
          <PrimaryButton type="button" disabled={isSubmitting} onClick={() => void handleSubmit()}>
            ...
            <span>
              {isSubmitting
                ? INTERVIEWER_UI_TEXT.SUBMITTING
                : INTERVIEWER_UI_TEXT.SUBMIT_FEEDBACK}
            </span>
          </PrimaryButton>
        </Actions>
      </Card>
