...
export const PendingApprovalsPanel = ({ isActive }: PendingApprovalsPanelProps) => {
  const { pendingUsers, isUpdatingApproval, isPendingApprovalsLoading, onApprove } =
    usePendingApprovals(isActive);
  ...
  if (isPendingApprovalsLoading) {
    return (
      <DashboardDescription>
        {HR_INTERVIEW_DEFAULT_MESSAGES.PENDING_APPROVALS_LOADING}
      </DashboardDescription>
    );
  }
