...
export const useJobsPanel = ({ role, activePanelId }: JobsPanelProps) => {
  ...
  const [manageSearchQuery, setManageSearchQuery] = useState<string>(
    JOBS_INITIAL_VALUES.EMPTY_STRING,
  );
  const [selectedApplicationsJobId, setSelectedApplicationsJobId] = useState<string>(
    JOBS_INITIAL_VALUES.EMPTY_STRING,
  );
  const [applicationsJobsFilter, setApplicationsJobsFilter] = useState<JobsStatusFilter>('all');
  const [applicationsJobsSearchQuery, setApplicationsJobsSearchQuery] = useState<string>(
    JOBS_INITIAL_VALUES.EMPTY_STRING,
  );
  const [applicationsStatusFilter, setApplicationsStatusFilter] = useState<
    'all' | ApplicationStatus
  >('all');
  const [applicationsSearchQuery, setApplicationsSearchQuery] = useState<string>(
    JOBS_INITIAL_VALUES.EMPTY_STRING,
  );
  ...
  const [scheduleFormValues, setScheduleFormValues] =
    useState<ScheduleInterviewFormValues>(initialScheduleFormValues);
  ...
  const [selectedApplicationForStatusChange, setSelectedApplicationForStatusChange] =
    useState<HrJobApplication | null>(null);
  const [selectedStatusForChange, setSelectedStatusForChange] =
  ...
  const { data: hrJobsResponse, isLoading: isHrJobsLoading } = useGetHrJobsQuery(undefined, {
    skip: !isHr,
  });
  const { data: candidateJobsResponse, isLoading: isCandidateJobsLoading } =
    useGetCandidateJobsQuery(undefined, { skip: !isCandidate });
  const { data: appliedJobsResponse } = useGetAppliedJobsQuery(undefined, { skip: !isCandidate });
  const { data: interviewersResponse } = useGetInterviewersQuery(
    { page: 1, limit: 100 },
    { skip: !isHr },
  );
  const { data: jobApplicationsResponse, isLoading: isJobApplicationsLoading } =
    useGetJobApplicationsQuery(
  ...
  const hrJobApplications = useMemo(
    () => jobApplicationsResponse?.data ?? [],
    [jobApplicationsResponse?.data],
  );
  ...
  const filteredAllJobs = useMemo(() => {
    ...
    return visibleJobs.filter((job) => {
      const matchesStatus =
        allJobsFilter === 'all' ||
        (allJobsFilter === 'active' && job.isActive) ||
        (allJobsFilter === 'closed' && !job.isActive);
      ...
      const searchable = [job.title, job.requiredSkills, job.experienceLevel, job.description]
        .join(' ')
        .toLowerCase();
  ...
  const filteredApplicationsJobs = useMemo(() => {
    ...
    return visibleJobs.filter((job) => {
      ...
      const searchable = [job.title, job.requiredSkills, job.experienceLevel]
        .join(' ')
        .toLowerCase();
      return searchable.includes(normalizedQuery);
    });
  ...
  const jobsEmptyMessage =
    isHrManageView && allJobsFilter !== 'all'
      ? JOBS_PANEL_TEXT.NO_FILTERED_JOBS(allJobsFilter)
      : JOBS_PANEL_TEXT.NO_JOBS;
  ...
  const filteredHrJobApplications = useMemo(() => {
    ...
    return hrJobApplications.filter((application) => {
      ...
      const matchesStatus =
        applicationsStatusFilter === 'all' || applicationsStatusFilter === normalizedStatus;
      if (!matchesStatus) {
        return false;
      }
  ...
  const selectedApplicationForAction = actionMenuAnchor
    ? (hrJobApplications.find(
        (application) => application._id === actionMenuAnchor.applicationId,
      ) ?? null)
    : null;
