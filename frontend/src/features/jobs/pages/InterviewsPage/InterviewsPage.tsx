...
import { INTERVIEWS_FILTER_LABELS, JOBS_DEFAULT_MESSAGES, JOBS_UI_TEXT } from "../../labels/jobLabels";
...
export const InterviewsPage = ({ initialView = "both" }: InterviewsPageProps) => {
  const { interviews, filteredInterviews, isLoading, statusFilter, setStatusFilter, searchQuery, setSearchQuery } =
  ...
  if (interviews.length === 0) {
    return <EmptyStateCard title={JOBS_DEFAULT_MESSAGES.NO_INTERVIEWS_TITLE} description={JOBS_DEFAULT_MESSAGES.NO_INTERVIEWS_DESCRIPTION} />;
  }
    ...
    <PageWrap>
      <Section>
        <StickyHeader>
          ...
          <ControlsRow>
            ...
            <FiltersRow>
              ...
              <FilterButton type="button" $active={statusFilter === "upcoming"} onClick={() => setStatusFilter("upcoming")}>
        ...
        {filteredInterviews.length === 0 ? (
          <EmptyStateCard title={JOBS_DEFAULT_MESSAGES.NO_INTERVIEWS_FOUND_TITLE} description={JOBS_DEFAULT_MESSAGES.NO_INTERVIEWS_FOUND_DESCRIPTION} />
        ) : null}
        <CardsGrid>
