...
import {
  ...
  JOBS_UI_TEXT,
  MY_APPLICATIONS_FILTER_OPTIONS,
...
import {
  formatJobsDate,
  getJobTitleFallback,
  normalizeApplicationDisplayStatus,
...
export const MyApplicationsPage = () => {
  ...
  if (isLoading) {
    return (
      <DashboardDescription>{JOBS_DEFAULT_MESSAGES.LOADING_APPLICATIONS}</DashboardDescription>
    );
  }
  ...
  if (applications.length === 0) {
    return (
      <EmptyStateCard
        title={JOBS_DEFAULT_MESSAGES.NO_APPLICATIONS_TITLE}
        description={JOBS_DEFAULT_MESSAGES.NO_APPLICATIONS_DESCRIPTION}
      />
    );
  }
  ...
  return (
    <PageWrap>
      <StickyHeader>
        ...
        <ControlsRow>
          ...
          <FiltersRow ref={filterRef}>
            ...
            >
              {MY_APPLICATIONS_FILTER_OPTIONS.find((option) => option.value === statusFilter)
                ?.label ?? JOBS_UI_TEXT.APPLICATIONS_ALL_LABEL}
            </FilterSelectButton>
            {isFilterOpen ? (
      ...
      {applications.length === 0 ? (
        <EmptyStateCard
          title={JOBS_DEFAULT_MESSAGES.NO_APPLICATIONS_FOUND_TITLE}
          description={JOBS_DEFAULT_MESSAGES.NO_APPLICATIONS_FOUND_DESCRIPTION}
        />
      ) : null}
      <CardsGrid>
