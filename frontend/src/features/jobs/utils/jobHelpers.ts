...
export const normalizeApplicationDisplayStatus = (status: string): string =>
  status.replace(/_/g, " ").toLowerCase().replace(/\b\w/g, (character) => character.toUpperCase());

export const toInterviewDateTime = (date: string, time: string): Date => new Date(`${date}T${time}:00+05:30`);

export const getAppliedJobId = (jobId: { _id?: string } | string): string | null =>
  typeof jobId === "string" ? jobId : jobId?._id ?? null;

export const getJobTitleFallback = (title?: string): string => title ?? JOBS_DEFAULT_MESSAGES.JOB_FALLBACK_TITLE;
