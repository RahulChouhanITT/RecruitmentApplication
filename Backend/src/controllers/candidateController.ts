...
export const uploadResumeHandler = async (
  ...
  const updatedProfile = await uploadCandidateResume(req.authenticatedUserId as string, requestWithFile.file);
  ...
  sendSuccessResponse(res, {
    ...
    data: {
      ...
      resumePublicId: updatedProfile.resumePublicId,
    },
  });
};
...
export const deleteResumeHandler = async (
  ...
  const updatedProfile = await deleteCandidateResume(req.authenticatedUserId as string);
